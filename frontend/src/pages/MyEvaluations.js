              ...
              <Card className="rounded-2xl border-border hover:bg-secondary transition">
                <CardContent className="py-3.5 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground truncate">{ev.athlete?.first_name} {ev.athlete?.last_name}</p>
                    <p className="text-xs text-muted-foreground">{ev.station_name} · {(ev.updated_at || "").slice(0, 10)}</p>
                  </div>
                  {ev.computed?.overall_score != null && <span className="font-mono-num font-bold text-foreground">{ev.computed.overall_score}</span>}
