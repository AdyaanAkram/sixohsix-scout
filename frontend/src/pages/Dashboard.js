...
const StatCard = ({ label, value, icon: Icon, accent = "hsl(var(--foreground))", testId, to, sub }) => {
  const card = (
    <Card className={`rounded-2xl border-border h-full ${to ? "cursor-pointer transition-all hover:bg-secondary/50 hover:ring-1 hover:ring-ring/40" : ""}`} data-testid={testId}>
        ...
        <Card className="rounded-2xl border-border overflow-hidden">
          ...
          <CardContent className="pt-4 pb-5">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <StatCard label="Checked In" value={stats.checked_in} icon={CheckCircle2} accent="hsl(var(--success))" testId="stat-checked-in" to={`/events/${ev.id}?tab=checkin`} />
              <StatCard label="Evaluations Completed" value={stats.evaluations_completed} icon={ClipboardCheck} accent="hsl(var(--info))" testId="stat-evals-completed" to={`/events/${ev.id}?tab=progress`} />
              <StatCard label="Drafts In Progress" value={stats.evaluations_draft} icon={Activity} accent="hsl(var(--warning))" testId="stat-evals-draft" to={`/events/${ev.id}?tab=progress`} />
            </div>
          </CardContent>
        </Card>
      ) : (
