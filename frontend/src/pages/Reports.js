  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-4xl text-[#0B1E3A]">Reports</h1>
          <p className="text-sm text-slate-500">Internal rankings and completion reports. Not for public distribution.</p>
        </div>
        {eventId && (
      ...
      <div className="flex flex-wrap gap-2">
        <Select value={eventId} onValueChange={setEventId}>
          <SelectTrigger className="w-[240px] h-11 rounded-xl bg-white" data-testid="reports-event-select"><SelectValue placeholder="Select event" /></SelectTrigger>
          <SelectContent>{events.map((e) => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}</SelectContent>
        </Select>
        <Select value={ageGroup} onValueChange={setAgeGroup}>
          <SelectTrigger className="w-[110px] h-11 rounded-xl bg-white" data-testid="reports-age-filter"><SelectValue /></SelectTrigger>
        ...
        <Select value={position} onValueChange={setPosition}>
          <SelectTrigger className="w-[130px] h-11 rounded-xl bg-white" data-testid="reports-position-filter"><SelectValue /></SelectTrigger>
        ...
        <Select value={category} onValueChange={setCategory}>
          <SelectTrigger className="w-[150px] h-11 rounded-xl bg-white" data-testid="reports-category-filter"><SelectValue /></SelectTrigger>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="rounded-xl bg-[hsl(var(--secondary))] h-11">
          <TabsTrigger value="leaderboard" className="rounded-lg px-4 data-[state=active]:bg-[#0B1E3A] data-[state=active]:text-white" data-testid="reports-tab-leaderboard">Leaderboard</TabsTrigger>
          <TabsTrigger value="completion" className="rounded-lg px-4 data-[state=active]:bg-[#0B1E3A] data-[state=active]:text-white" data-testid="reports-tab-completion">Completion</TabsTrigger>
          <TabsTrigger value="disagreement" className="rounded-lg px-4 data-[state=active]:bg-[#0B1E3A] data-[state=active]:text-white" data-testid="reports-tab-disagreement">Disagreement</TabsTrigger>
        </TabsList>
                ...
                <TableBody>
                  {leaderboard.map((r) => (
                    <TableRow key={r.athlete.id}>
                      <TableCell>
                        <span className={cn("font-display text-xl", r.rank <= 3 ? "text-[#F4B400]" : "text-slate-400")}>{r.rank}</span>
                      </TableCell>
                      ...
                      <TableCell className="text-slate-600">{r.athlete.current_team || "—"}</TableCell>
                      <TableCell className="text-right font-mono-num font-bold">{r.score}</TableCell>
                      <TableCell className="text-right font-mono-num text-slate-500">{r.evaluation_count}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <TabsContent value="completion" className="mt-4">
          {!completion ? <Skeleton className="h-64 rounded-2xl" /> : (completion.rows || []).length === 0 ? (
            ...
            <Card className="rounded-2xl card-shadow border-[#E7E1D6] overflow-hidden">
                  ...
                  <TableBody>
                    {completion.rows.map((r) => (
                      <TableRow key={r.athlete.id}>
                        ...
                        <TableCell><span className={cn("text-xs font-semibold", r.check_in_status === "checked_in" ? "text-[#1F7A4D]" : "text-slate-400")}>{r.check_in_status === "checked_in" ? "In" : r.check_in_status}</span></TableCell>
                        {(completion.station_names || []).map((s) => {
                          const st = r.stations[s];
                          return (
                            <TableCell key={s} className="text-center">
                              <span className={cn("inline-block h-2.5 w-2.5 rounded-full",
                                st === "complete" ? "bg-[#1F7A4D]" : st === "draft" ? "bg-[#F4B400]" : st === "missing" ? "bg-[#C81D25]" : "bg-slate-200")} title={st} />
                            </TableCell>
                          );
                        })}
                      </TableRow>
              ...
              <div className="px-4 py-3 border-t flex gap-4 text-xs text-slate-500">
                <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#1F7A4D] inline-block" /> Complete</span>
                <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#F4B400] inline-block" /> Draft</span>
                <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#C81D25] inline-block" /> Missing</span>
          ...
          {!disagreement ? <Skeleton className="h-64 rounded-2xl" /> : disagreement.length === 0 ? (
            ...
            <div className="space-y-2">
              {disagreement.map((d, i) => (
                <Card key={i} className="rounded-2xl border-[#E7E1D6]">
                  <CardContent className="py-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <Link to={`/players/${d.athlete?.id}`} className="font-semibold text-[#0B1E3A] hover:underline">{d.athlete?.first_name} {d.athlete?.last_name}</Link>
                        <p className="text-xs text-slate-500">{d.station_name}</p>
                      </div>
                      <span className="font-mono-num font-bold text-[#C81D25]">Δ {d.spread}</span>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {d.scores.map((s, j) => (
                        <span key={j} className="rounded-full bg-[hsl(var(--secondary))] px-3 py-1 text-xs font-medium">{s.evaluator}: <span className="font-mono-num font-bold">{s.score}</span></span>
                      ))}
                    </div>
                  </CardContent>
                </Card>
